        ...
        <div class="lg:col-span-2 space-y-4 md:space-y-6 order-2">
          ...
          <div class="bg-white rounded-lg shadow p-4 sm:p-6">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-4">
              ...
              <button v-if="!editingPersonal" @click="startEditPersonal"
            ...
            <form v-if="editingPersonal" @submit.prevent="savePersonal">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input v-model="personalForm.user.first_name" type="text"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label class="block text-gray-700 font-semibold mb-2">Apellido Paterno</label>
                  <input v-model="personalForm.user.last_name" type="text"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label class="block text-gray-700 font-semibold mb-2">Apellido Materno</label>
                  <input v-model="personalForm.user.middle_name" type="text"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label class="block text-gray-700 font-semibold mb-2">Teléfono</label>
                  <input v-model="personalForm.user.phone" type="tel"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>

              <div v-if="personalSuccess" class="mb-3 p-3 bg-green-100 text-green-700 rounded-lg text-sm">
                Información actualizada correctamente.
              </div>

              <div class="flex flex-col-reverse sm:flex-row gap-3">
                <button type="button" @click="cancelPersonal"
          ...
          <div class="bg-white rounded-lg shadow p-4 sm:p-6">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-4">
              ...
              <button v-if="!editingSchedule" @click="startEditSchedule"
                class="text-blue-600 hover:text-blue-800 font-semibold text-sm">
                Editar
              </button>
            </div>
            ...
            <form v-if="editingSchedule" @submit.prevent="saveSchedule">
              ...
              <div v-if="scheduleSuccess" class="mb-3 p-3 bg-green-100 text-green-700 rounded-lg text-sm">
          ...
          <div class="bg-white rounded-lg shadow p-4 sm:p-6">
            ...
            <form v-if="editingPassword" @submit.prevent="savePassword">
              ...
              <div v-if="passwordError" class="mb-3 p-3 bg-red-100 text-red-700 rounded-lg text-sm">
                {{ passwordError }}
              </div>
              <div v-if="passwordSuccess" class="mb-3 p-3 bg-green-100 text-green-700 rounded-lg text-sm">
...
async function savePersonal() {
  ...
  error.value = "";
  try {
    const updatedProfile = await psychologistsStore.updatePsychologist(
      profile.value.id,
      personalForm.value,
    );
